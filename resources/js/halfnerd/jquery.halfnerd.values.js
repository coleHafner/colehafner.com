...
$( document ).ready( function(){
    ...
    $( "#authentication" )
		...
		switch( process.toLowerCase() )
		{
			...
			default:
				//show colorbox
				$.colorbox({ href:'/resources/ajax/halfnerd_helper.php?task=single_value&process=' + process + '&single_val_id=' + single_val_id });
				break;
		}
	});
...
**********************************************************************************************************************************/
	...
	this.add = function()
	{
		$.ajax({
			type: 'post',
			url: '/resources/ajax/halfnerd_helper.php?task=single_value&process=add',
	...
	this.modify = function( single_val_id )
	{
		$.ajax({
			type: 'post',
			url: '/resources/ajax/halfnerd_helper.php?task=single_value&process=modify&single_val_id=' + single_val_id,
			data: $( "#single_val_form" ).serialize( true ),
			success: function( reply ){
				reloadPage( 0 );
			}
	...
	**********************************************************************************************************************************/
	...
	this.validateAddModForm = function( process, single_val_id )
	{
		$.ajax({
			type: 'post',
			url: '/resources/ajax/halfnerd_helper.php?task=single_value&process=validate',
			data: $( "#single_val_form" ).serialize( true ),
			success: function( reply ) {
